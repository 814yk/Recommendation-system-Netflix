...
def item_rating_pred(path, rating,method,k):
    start = time.time()
    name='item'
    ...
    if method =='dot':
        ...
        item_mtx = cos_sim(inputs,name) #honestly this is dot product but input is normalized so same with cos_similarity
    #KNN
    for i in data:
        score = 0
